...
int main () {
	...
	*/
	int element_size;
	for (int i = 0; i <= 1; i++) {//a loop for the different block lengths
		int numBlocks = 16384; 
		int block_len;
		...
		cudaMalloc(&out , numBlocks * block_len * element_size);
		cudaMalloc(&in , numBlocks * block_len * element_size);	
			

		// Make a warmup
		if (element_size == 1) {
		copyKernel<char><<<numBlocks, block_len>>>(out, in);
		cudaDeviceSynchronize();
		}
		....
